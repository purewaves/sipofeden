...
async function createTables() {
  ...
  try {
    ...
    const adminResult = await sql`SELECT 1 FROM admins WHERE username = 'admin' LIMIT 1`;
    ...
    const settingsResult = await sql`SELECT 1 FROM website_settings LIMIT 1`;
    ...
    const juicesResult = await sql`SELECT 1 FROM juices LIMIT 1`;
    if (juicesResult.length === 0) {
      await sql`
    ...
    console.log("create-tables script finished.");
  }
}
